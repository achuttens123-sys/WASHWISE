    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      {user?.userType === 'subscriber' && !user?.subscriptionPaid && (
        <motion.div
          ...
          className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-2xl mb-8 flex items-center justify-between"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-amber-800 dark:text-amber-100 text-sm">Subscription Payment Required</p>
              <p className="text-amber-600 dark:text-amber-400 text-xs">Please complete your payment to start booking slots.</p>
            </div>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="md:col-span-3">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 dark:shadow-blue-900/20">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-1">
                  <p className="text-gray-500 dark:text-gray-400">Book your laundry slot below</p>
                  {user?.userType === 'subscriber' && user?.package && user?.subscriptionPaid && (
                    <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest rounded-full">
                      {user.package} Plan
                    </span>
                  )}
                  <button 
                    onClick={() => setShowTerms(true)}
                    className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline transition-all flex items-center gap-1"
                  >
            ...
            <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
              <button
                ...
                className={`px-6 py-2 rounded-lg font-medium transition-all ${view === 'today' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
              >
                Today
              </button>
              <button
                onClick={() => setView('advance')}
                className={`px-6 py-2 rounded-lg font-medium transition-all ${view === 'advance' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
              >
                Advance
              </button>
            </div>
        ...
        <div className="md:col-span-1">
          <motion.div 
            ...
            className="bg-indigo-600 dark:bg-indigo-700 p-6 rounded-[2rem] text-white shadow-xl shadow-indigo-200 dark:shadow-indigo-900/20 relative overflow-hidden group"
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest opacity-80">Loyalty Points</span>
              </div>
              <p className="text-4xl font-black tracking-tighter mb-1">{user?.points || 0}</p>
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">Available Balance</p>
              
              <div className="mt-4 pt-4 border-t border-white/10">
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-1">Next Free Wash</p>
            ...
            {activeBookings.map((booking) => {
              ...
              return (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-white dark:bg-gray-900 p-6 rounded-[2rem] border border-blue-50 dark:border-gray-800 shadow-xl shadow-blue-100/20 dark:shadow-none"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-blue-600 rounded-2xl shadow-lg shadow-blue-200 dark:shadow-none">
                        <Loader2 className={`w-6 h-6 text-white ${['In Wash', 'In Drier'].includes(booking.status) ? 'animate-spin' : ''}`} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-xs font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">{booking.serviceType}</p>
                          <span className="text-[10px] font-black text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-lg">M#{booking.machineNumber}</span>
                        </div>
                        <h3 className="text-xl font-black text-gray-800 dark:text-gray-100 tracking-tight">{booking.status.toUpperCase()}</h3>
                      </div>
                    </div>
                    <div className="text-left md:text-right">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Scheduled For</p>
                      <p className="text-sm font-black text-gray-700 dark:text-gray-300">{booking.date} • {booking.timeSlot}</p>
                    </div>
                  </div>
                      ...
                      {steps.map((step, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex flex-col items-center group">
                            ...
                            <p className={`mt-3 text-[10px] font-black uppercase tracking-tighter text-center max-w-[60px] transition-colors duration-500 ${
