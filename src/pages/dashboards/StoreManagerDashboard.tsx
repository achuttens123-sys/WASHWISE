...
const StoreManagerDashboard: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'today' | 'machines' | 'status' | 'staff' | 'logistics' | 'issues'>('today');
    ...
    <div className="space-y-8">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'staff' && (
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-black text-gray-800 dark:text-gray-100 uppercase tracking-tight">Store Staff</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {storeStaff.length === 0 ? (
                <div className="col-span-full bg-blue-50 dark:bg-blue-900/20 p-8 rounded-[3rem] border border-blue-100 dark:border-blue-800/30 text-center">
                  <Users className="w-12 h-12 text-blue-600 mx-auto mb-4" />
                  <h3 className="text-xl font-black text-blue-800 dark:text-blue-200 uppercase tracking-tight mb-2">No Staff Found</h3>
                  <p className="text-blue-600/70 dark:text-blue-400/70 max-w-md mx-auto font-medium">
                    There are no staff members assigned to this store yet.
                  </p>
                </div>
              ) : (
                storeStaff.map((staff) => (
                  <div key={staff.uid} className="bg-white dark:bg-gray-900 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-2xl flex items-center justify-center">
                        <User className="w-6 h-6 text-gray-400" />
                      </div>
                      <div>
                        <h4 className="font-black text-gray-800 dark:text-gray-100 uppercase tracking-tight">{staff.name}</h4>
                        <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">{staff.adminRole?.replace('_', ' ')}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-400 font-bold uppercase tracking-widest">Staff ID</span>
                        <span className="font-black text-gray-800 dark:text-gray-100">{staff.staffId || 'N/A'}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-400 font-bold uppercase tracking-widest">Status</span>
                        <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${
                          staff.isRegistered ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                        }`}>
                          {staff.isRegistered ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                    </div>
                  </div>
